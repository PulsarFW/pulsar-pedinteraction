...
CreateThread(function()
		...
		plsr.Callbacks:RegisterServerCallback("Vendor:GetItems", function(source, data, cb)
			...
			if char ~= nil then
				if _created[data] ~= nil then
					...
					if _created[data] and not _illegalBlacklist[plsr.State:Player(source).onDuty or ""] then
							...
							if
								...
								and (
									...
									or v.requireCurrency
										and ((v.coin ~= nil and plsr.Crypto:Has(source, v.coin, v.price)) or (v.coin == nil and plsr.Wallet:Has(
											source,
											v.price
										)))
								)
						...
						plsr.Logger:Warn(
							...
							string.format(
								...
								char:GetData("SID"),
								data,
								plsr.State:Player(source).onDuty
							)
						)
						cb({})
					end
		...
		plsr.Callbacks:RegisterServerCallback("Vendor:BuyItem", function(source, data, cb)
			local char = plsr.Fetch:CharacterSource(source)
			if char ~= nil then
				if _created[data.id] ~= nil then
					if not _created[data.id].stockTimeDelay or os.time() > _created[data.id].stockTimeDelay then
						...
						if _created[data.id] and not _illegalBlacklist[plsr.State:Player(source).onDuty or ""] then
							if itemData ~= nil and (itemData.qty == -1 or itemData.qty > 0) then
								if
									(
										itemData.rep == nil
										or plsr.Reputation:GetLevel(source, itemData.rep) >= (itemData.repLvl or 1)
									)
									...
									and (
										...
										or _created[data.id].requireCurrency
											and ((itemData.coin ~= nil and plsr.Crypto:Has(
												source,
												itemData.coin,
												itemData.price
											)) or plsr.Wallet:Has(source, itemData.price))
									)
								then
									if itemData.coin ~= nil then
										local coinData = plsr.Crypto.Coin:Get(itemData.coin)
										if
										...
										then
											...
											cb(plsr.Inventory:AddItem(char:GetData("SID"), itemData.item, 1, {}, 1))
										else
											plsr.Execute:Client(
												source,
												"Notification",
										...
										if plsr.Wallet:Has(source, itemData.price) then
											if plsr.Wallet:Modify(source, -itemData.price) then
												if itemData.qty ~= -1 then
													_created[data.id].items[data.index].qty = itemData.qty - 1
												end
...
_VENDORS = {
	Create = function( self, id, type, name, model, position, items, iconOverride, labelOverride, isUnique, isGlobalUnique, isIllegal, stockTimeDelay, restock )
...
AddEventHandler("Proxy:Shared:RegisterReady", function()
	exports["pulsar_core"]:RegisterComponent("Vendor", _VENDORS)
end)
